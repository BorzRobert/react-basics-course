...
export function PureFunctions() {
  ...
  return (
    <div style={{ display: "grid", gap: 12, maxWidth: 420 }}>
      <p style={{ margin: 0 }}>
        Introdu sumă în RON și vezi cât primești în EUR. Cursul poate fi schimbat din interfață, ca într-un caz real.
      </p>

      <label htmlFor="ron-input" style={{ fontWeight: 600 }}>
        Sumă în RON
      </label>
      <input id="ron-input" type="number" min="0" value={ron} onChange={event => updateRon(event.target.value)} />
      ...
      <div style={{ border: "1px solid #ccc", padding: 12, borderRadius: 6 }}>
        ...
        <p style={{ margin: 0 }}>Pure: {pureValue.toFixed(2)} EUR</p>
        <p style={{ margin: 0 }}>Impure: {impureValue.toFixed(2)} EUR</p>
      </div>

      <p style={{ margin: 0, fontSize: 13 }}>
        Observă: schimbarea sumei sau a cursului re-randează componenta. Bifează comisionul și vezi că valoarea din UI
        rămâne veche până la următorul render.
      </p>
    </div>
  );
}
